...
class IndustryServicePay extends React.Component {
  state = {
    cardNum: "",
    cardExpiryMonth: "",
    cardExpiryYear: "",
    ...
    return (
      <Query query={CURRENT_USER}>
        {({ loading, error, data }) => {
          ...
          return (
            <Mutation
              mutation={INDUSTRIAL_WASTE}
              awaitRefetchQueries={true}
              refetchQueries={[{ query: WASTE_COLLECTION_QUOTES }]}
            >
              {(createIndustrialQuote, { loading }) => (
                <Formik
                  initialValues={{
                    cardNum: "",
                    cardExpiryMonth: "",
                  ...
                  onSubmit={values => {
                    ...
                    RNPaystack.chargeCard({
                      ...
                      cvc: values.cardCvc,
                      amountInKobo: item.amount * 100,
                      email: user.email
                    })
                      .then(res => {
                        console.log(item.address.typeOf);
                        createIndustrialQuote({
                          variables: {
                            subscriptionType: "Industrial Waste Collection",
                            amount: item.amount,
                        ...
                        this.props.navigation.navigate("WasteSubscription");
                        return res;
                      })
                      .catch(error => {
                        this.setState({ loading: false });
...
export default IndustryServicePay;
