...
RNPaystack.init({
  publicKey: "pk_live_834b1353913ff1d0cd0f4c1ff6c1184727f90df5"
});
import ErrorPage from "../ErrorPage";

class EvacuationPay extends React.Component {
  state = {
    cardNum: "",
    cardExpiryMonth: "",
    cardExpiryYear: "",
    ...
    return (
      <Query query={CURRENT_USER}>
        {({ loading, error, data }) => {
          ...
          return (
            <Mutation
              mutation={SEWAGE_EVACUATION}
              awaitRefetchQueries={true}
              refetchQueries={[{ query: EVACUATION_QUOTE }]}
            >
              {(CreateEvacuationQuote, { loading }) => (
                <Formik
                  initialValues={{
                    cardNum: "",
                    cardExpiryMonth: "",
                    ...
                    RNPaystack.chargeCard({
                      ...
                      .then(res => {
                        CreateEvacuationQuote({
                          variables: {
                            subscriptionType: "Sewage Evacuation",
                            amount: item.amount,
                            serviceStartDate: item.date.toString().slice(0, 16),
